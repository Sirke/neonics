...
TerriETHlow= -0.38 - 0.97*year - 0.16*Abuild + 0.24*Aroad + 0.07*Astream + 0.30*Ariver + 0.56*agri5 +1.48*LOU+
  ...
  0.15*prec*year+ mean(dataS$Larea)
TerriETHlow
a=exp(TerriETHlow)
...
TerriETHmed= -0.38 - 0.97*year - 0.16*Abuild + 0.24*Aroad + 0.07*Astream + 0.30*Ariver + 0.56*agri5 +1.48*LOU+
  1.99*POH + 1.47*UUS - 0.13*((NeoPrior-0.069)/(2*0.115447)) + 0.14*temp - 0.05*prec + 0.03*year*NeoPrior + 0.08*temp*year - 0.15*prec*year + mean(dataS$Larea)
TerriETHmed
a=exp(TerriETHmed)
...
confETHlow= -0.99 - 1.22*year - 0.48*Abuild - 0.07*Aroad - 0.19*Astream + 0.06*Ariver + 0.27*agri5 +0.79*LOU+
  ...
  0.31*prec*year+ mean(dataS$Larea)
confETHlow
a=exp(confETHlow)
...
confETHlow= 0.23 - 0.71*year +0.17*Abuild +0.55*Aroad +0.33*Astream + 0.54*Ariver + 0.84*agri5 +2.17*LOU+
  2.76*POH + 2.14*UUS - 0.01*((NeoPrior-0.069)/(2*0.115447)) + 0.23*temp +0.04*prec +0.29*year*NeoPrior +0.28*temp*year +0.01*prec*year+ mean(dataS$Larea)
confETHlow
a=exp(confETHlow)
a
#uus
...
*Käsin laskettaessa ennusteet pystyy laskemaan conditional average -estimaateilla (predict funktiolla on pakko käyttää full average -estimaatteja). Tosin luettuani tämän [vastauksen ](https://stackoverflow.com/questions/53055050/predicted-values-with-mumin-throwing-error-when-full-false), en olekaan enää niin varma siitä, kumpia arvoja ennusteiden tekemisessä pitäisi käyttää.
...
#tarkistetaan ettei globaalimalli ole ylidispersoitunut
E2 <- resid(M9, type = "pearson")
N  <- nrow(dataS)
p  <- 23 + 3  #2 sigmas and 1 size/k/theta NB parameter
sum(E2^2) / (N - p)
#globaalimalli on hieman alidispersoitunut, 0.74, onko tämä ongelma?
...
SE:t kertovat kuinka kauas oikeasta populaation keskiarvosta sun otoksien keskiarvot sijoittuvat. Kertoo siis otosvirheestä (vaihtelusta mitä otokset sisältävät, otoskoko vaikuttaa).
...
95 % luottamusväli kertoo 

## ennusteet predict-funktiolla

```{r}
#predictions
newdata <- expand.grid(NeoPrior= with(dataS, c(0.13, 1.85, 3.58)),
                       year = mean(dataS$year),
                       Abuild = mean(dataS$Abuild),
                       Aroad = mean(dataS$Aroad),
                       Astream = mean(dataS$Astream),
...
newdata$NeoPrior=revalue(newdata$NeoPrior,c("0.13"="low","1.85"="med","3.58"="high"))
